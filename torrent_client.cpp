...
string create_mtorrent_file(string filename, string mtorrent_file, string tracker_ip, string tracker_port){
  ...
  while((bytes_read = (fread(data, sizeof(unsigned char), chunk_size, in)) ) > 0){
    ...
    int i;
    for(int j = 0; j < strlen((char *)obuf); j++){
      if(obuf[j] == '|' || obuf[j] == ';')
        obuf[j] = 'a';
    }
    for (i = 0; i < 20; i++) {
      ...
      snprintf(hash_concat_pieces, sizeof hash_concat_pieces, "%02x", obuf[i]);
    }
  ...
  char *buf = (char *)malloc(sizeof(char) * strlen(hash_concat_pieces));
  strcpy(buf, hash_concat_pieces);
  strcat(buf, "\0");
  //cout<<data;
  SHA1((unsigned char *)&hash_concat_pieces, strlen(buf),(unsigned char *)final_hash);
  for (int i = 0; i < 20; i++) {
    //cout<<hex<<obuf[i]<<" ";
  ...
  //cout<<"Read:"<<fread(data, sizeof(unsigned char), 1024, in);
  //cout<<data;
  //cout<<hash<<endl;
  fclose(in);
  fclose(out);
...
int share_file(string filename, string mtorrent_file, string tracker_ip, string tracker_port, string client_ip,string upload_port){
  ...
  send_data += ";" + client_ip + ";" + upload_port + ";" + filename + ";" + "a3f3f223aedfadcijfqyornaba34a13";
...
struct TorrentInfo get_torrent_info(string mtorrent_path){
  cout<<"Inside get torrentInfo"<<endl;
...
struct Client get_client_info(char *token){
  cout<<"Inside get client info"<<endl;
...
void * get_file_from_server(void *chunkdata ){
  ...
  for(int i = valread; i < chunk_size; i++)
    buffer[i] = 0;
  string name = to_string(chunk_data.chunk_number) + "_" + chunk_data.filename ;


  FILE *in = fopen(name.c_str() , "w");
  for(int i = 0; i < valread; i++)
    fprintf(in, "%c", buffer[i]);
  fclose(in);
...
void * download_file(void *d){
  ...
  int chunks_to_fetch = ceil(info.size/(float)chunk_size);
  ...
  for(int i = 0; i < chunks_to_fetch; i++){
    ...
    chunkdata.chunk_number = i;
    chunkdata.ip = seeder_list[server].client_ip;
    chunkdata.port = atoi(seeder_list[server].client_port.c_str());
    chunkdata.filename = info.filename;
    server++;
    pthread_t tid;
    cout<<"Requesting chunk number:"<<i<<endl;
    pthread_create(&tid, NULL, get_file_from_server, (void *)(&chunkdata) );
  ...
  /*FILE *out = fopen(info.filename, "w");
